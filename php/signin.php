<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" and isset($_POST['email']) and isset($_POST['password'])) {
        ...
        $sql = "SELECT `_uid`, `name`, `email`, `username`, `number`, `image`, `active` FROM `users` WHERE (`email`='$email' OR `username`='$email') AND `password`=MD5('$password')";
        require 'db.php';
        ...
        if ($result != false) {
            if ($result->num_rows > 0) {
                ...
                if ($u_data['active'] == 1) {
                    ...
                    $url = "https://wampinfotech.com/wamp/php/activate.php?hash=$hash&uid=$uid";
